...
/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the poptip.
 ...
 - `none`: No Animation
 */
public enum ToolTipEntranceAnimation {
...
/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the poptip.
 ...
 - `none`: No Animation
 */
public enum ToolTipExitAnimation {
...
/** Enum that specifies the type of action animation. Action animations are performed after the poptip is visible and the entrance animation completed.
 ...
 - `none`: No animation
 */
public enum ToolTipActionAnimation {
...
open class ToolTip: UIView {
    ...
    open private(set) var direction = ToolTipDirection.none
    ...
    fileprivate func setup() {
        ...
        if tapGestureRecognizer == nil {
            tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(ToolTip.handleTap(_:)))
            tapGestureRecognizer?.cancelsTouchesInView = true
            self.addGestureRecognizer(tapGestureRecognizer ?? UITapGestureRecognizer())
        }
        if tapRemoveGestureRecognizer == nil {
            tapRemoveGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(ToolTip.handleTapRemove(_:)))
        }
        if swipeGestureRecognizer == nil {
            swipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(ToolTip.handleSwipe(_:)))
        }
        
        if isApplicationInBackground == nil {
            NotificationCenter.default.addObserver(self, selector: #selector(ToolTip.handleApplicationActive), name: Notification.Name.UIApplicationDidBecomeActive, object: nil)
            NotificationCenter.default.addObserver(self, selector: #selector(ToolTip.handleApplicationResignActive), name: Notification.Name.UIApplicationWillResignActive, object: nil)
        }
    }
    ...
    open override func draw(_ rect: CGRect) {
        ...
        let path = ToolTip.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, borderWidth: borderWidth, radius: cornerRadius)
    ...
    open func show(text: String, direction: ToolTipDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        attributedText = nil
        self.text = text
        accessibilityLabel = text
    ...
    ///   - frame: The originating frame. The poptip's arrow will point to the center of this frame.
    ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
    open func show(attributedText: NSAttributedString, direction: ToolTipDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        text = nil
    ...
    ///   - frame: The originating frame. The poptip's arrow will point to the center of this frame.
    ///   - duration: Optional time interval that determines when the poptip will self-dismiss.
    open func show(customView: UIView, direction: ToolTipDirection, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
        text = nil
        attributedText = nil
        self.direction = direction
        containerView = view
    ...
    fileprivate func show(duration: TimeInterval? = nil) {
        ...
        performEntranceAnimation {
            ...
            if let duration = duration {
                self.dismissTimer = Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(ToolTip.hide), userInfo: nil, repeats: false)
            }
        }
    }
